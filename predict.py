...
# Merge regimes into main DataFrame
...
# Build KMeans & GMM transition‐prob features
...
# Final feature list & clean data
print("4) Preparing and cleaning data for consistent splitting...")
ALL_MODEL_FEATURES = FEATURES + KM_FEATS + GMM_FEATS
...
# Build arrays & time‐ordered split
...
for state in np.unique(reg_dev_km):
...
if per_km_regime_models_trained:
    for i, r_test_km in enumerate(reg_test_km):
...
# Train Per-GMM-Regime Models
print("\n10b) Training per-GMM-regime models using best feature set...")
models_reg_gmm = {}
reg_dev_gmm = df_dev_clean["vol_state"].values
for state_val in np.unique(reg_dev_gmm):
...
if per_gmm_regime_models_trained:
    for i, r_test_gmm in enumerate(reg_test_gmm):
...
# KMeans Regime‐specific Test RMSE
